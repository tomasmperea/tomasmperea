...
function suggestTripType(trip, items, ctx) {
  ...
  // ¿Hay reservas en firme que este clasificador no supo leer? Si las hay y
  // termina decidiendo por lo escrito, eso se dice: callarlo es el mismo
  // silencio que hizo que el PM viera "se guía por los vuelos y después no".
  var hayFirmes = destinosDelViaje(trip, items).deReservas;
...
function destinosDelViaje(trip, items) {
  ...
  function sumar(texto, fuente, cuando, firme) {
    var t = String(texto == null ? "" : texto).trim();
    if (!t) return;
    var k = norm(t);
    if (vistos[k]) return;
    vistos[k] = true;
    lugares.push({ lugar:t, fuente:fuente, desde:cuando || "", firme:!!firme });
  }

  var porFecha = function (a, b) { return String(a.start || "").localeCompare(String(b.start || "")); };

  // DOS CAJONES, y la diferencia importa. La primera versión de esto tenía
  // uno solo: cualquier reserva con un lugar escrito "mandaba". La auditoría
  // lo volteó con un caso que ya existía en la app y yo no había mirado: un
  // viaje a Bariloche con destino bien escrito y UN traslado al Aeropuerto de
  // Ezeiza terminaba diciéndole al modelo que Ezeiza era lo que manda y que
  // Bariloche no iba "NUNCA por encima". El traslado al aeropuerto de salida
  // es de los datos más comunes que hay, y apuntaba al lugar de donde se sale.
  //
  // Lo que la app puede saber de cada campo, leyendo su propio extractor
  // (el prompt de importar dice que en un transfer `to` es "el lugar de
  // llegada tal como está escrito", sea el aeropuerto de salida o el centro
  // de Roma):
  //
  //   EN FIRME  vuelo (`to`)        aterrizar en un lugar es estar ahí.
  //             alojamiento (`address`)  una cama reservada es estar ahí.
  //   PISTA     traslado (`to`)     puede ser el viaje AL aeropuerto de casa.
  //             auto (`address`)    el retiro puede ser en el de salida.
  //
  // Las pistas se mandan igual —son información— pero no vuelven `deReservas`
  // verdadero, así que no desplazan el destino que la persona escribió. No es
  // una suposición sobre el viajero: es lo que el campo alcanza a decir.
  ...
  vuelos.forEach(function (f, k) {
    ...
    sumar(String(f.to).toUpperCase(), "vuelo", f.start, true);
  });

  items.filter(function (i) { return i.type === "stay" && i.address; }).sort(porFecha)
    .forEach(function (s) { sumar(s.address, "alojamiento", s.start, true); });

  items.filter(function (i) { return i.type === "transfer" && i.to; }).sort(porFecha)
    .forEach(function (t) { sumar(t.to, "traslado", t.start, false); });

  items.filter(function (i) { return i.type === "car" && i.address; }).sort(porFecha)
    .forEach(function (c) { sumar(c.address, "auto", c.start, false); });

  var deReservas = lugares.some(function (l) { return l.firme; });
  if (!deReservas) sumar(trip.destination, "escrito-a-mano", trip.startDate, false);

  return {
    lugares:lugares.filter(function (l) { return l.firme || l.fuente === "escrito-a-mano"; }),
    pistas:lugares.filter(function (l) { return !l.firme && l.fuente !== "escrito-a-mano"; }),
...
function lineaDestinos(b) {
  ...
  /* Las pistas —traslados y autos— se mandan SIEMPRE, con las dos lecturas
     posibles a la vista. Callarlas sería perder información; presentarlas como
     destino fue el error que volteó la auditoría. Que el modelo decida con el
     dato completo es mejor que decidir nosotros con medio dato. */
  var pistas = (d.pistas || []).map(function (l) { return "\"" + l.lugar + "\" (" + l.fuente + ")"; });
  var lineaPistas = pistas.length
    ? " Hay además " + pistas.join(" y ") + ": puede ser parte del viaje o el camino hasta el aeropuerto de salida, " +
      "así que no lo tomes como destino por sí solo."
    : "";

  if (!d.deReservas) {
    return "- Destino: " + partes.join(", ") +
           ". Lo escribió la persona al crear el viaje; no hay ningún vuelo ni alojamiento cargado que lo confirme." +
           lineaPistas;
  }
  var extra = (b.destino && norm(b.destino) !== norm(d.lugares[0].lugar))
    ...
    : "";
  return "- Destinos, sacados de los vuelos y alojamientos ya cargados, que son lo que manda: " + partes.join(" · ") +
